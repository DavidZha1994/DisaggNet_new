...
示例用法:
  ...
  python run_data_preparation.py --config config/prep_config.yaml --summary-only --output Data/prepared_data
    ...
    try:
        ...
        logger.info("已选择 HIPEDataPreparationPipeline（旧管线分支已移除）")
        ...
        print("\n2. 加载处理后的数据进行模型训练:")
        print(f"   from src.data_preparation import {pipeline.__class__.__name__}")
        print(f"   pipeline = {pipeline.__class__.__name__}('{args.config}')")
